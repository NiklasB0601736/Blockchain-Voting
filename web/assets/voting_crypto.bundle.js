var VotingCrypto = (() => {
  ...
  var voting_crypto_exports = {};
  __export(voting_crypto_exports, {
    aggregateEncryptedBallots: () => aggregateEncryptedBallots,
    committeeFingerprint: () => committeeFingerprint,
    createEncryptedVotePayload: () => createEncryptedVotePayload,
    createThresholdTallyPayload: () => createThresholdTallyPayload,
    deriveVoterCredentials: () => deriveVoterCredentials,
    encryptCandidateVote: () => encryptCandidateVote,
    generateCommittee: () => generateCommittee,
    hashText: () => hashText,
    validateCommitteeShare: () => validateCommitteeShare
  });
  ...
  var _Field = class {
    ...
    constructor(ORDER2, opts = {}) {
      if (ORDER2 <= _0n2)
        throw new Error("invalid field: expected ORDER > 0, got " + ORDER2);
    ...
    fromBytes(bytes, skipValidation = false) {
      abytes(bytes);
      const { _lengths: allowedLengths, BYTES, isLE, ORDER: ORDER2, _mod: modFromBytes } = this;
      if (allowedLengths) {
  ...
  // web/src/voting_crypto.js
  var CURVE_ID = 415;
  var CHALLENGE_DOMAIN = "voting-elgamal-chaum-pedersen-v1";
  var Point = p256.Point;
  var GENERATOR = Point.BASE;
  ...
  function hashText(value) {
    return bytesToHex2(sha256(encoder.encode(String(value))));
  }
  ...
  function evaluatePolynomial(coefficients, memberId) {
    ...
    for (const coefficient of coefficients) {
      result = mod2(result + coefficient * power);
      power = mod2(power * x);
    }
    return result;
  }
  ...
  function validateCommitteeShare(election, memberFile) {
    ...
    const expected = GENERATOR.multiply(mod2(BigInt(memberFile.share)));
  ...
  function challengeScalar(committee, publicShare, ciphertext, factor, t1, t2) {
    ...
    return bytesToBigInt(sha256(concatBytes2(transcript))) % ORDER;
  }
  function partialDecryption(committee, memberFile, ciphertextPayload, candidateIndex) {
    const share = mod2(BigInt(memberFile.share));
    const ciphertext = {
    ...
    const response = mod2(nonce + challenge * share);
    return {
  ...
  function lagrangeCoefficient(memberId, memberIds) {
    ...
    for (const otherId of memberIds) {
      ...
      numerator = mod2(numerator * -BigInt(otherId));
      denominator = mod2(denominator * BigInt(memberId - otherId));
    }
    return mod2(numerator * inverse(denominator));
  }
  ...
  function createThresholdTallyPayload(election, ballotPayloads, memberFiles) {
    const uniqueMembers = /* @__PURE__ */ new Map();
    ...
    for (let candidateIndex = 0; candidateIndex < encryptedTally.candidate_count; candidateIndex += 1) {
      ...
      const candidateShares = selected.map(
        (file) => partialDecryption(election.committee, file, ciphertextPayload, candidateIndex)
      );
  ...
  return __toCommonJS(voting_crypto_exports);
})();
