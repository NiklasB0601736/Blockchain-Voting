...
def create_app(blockchain_instance: Optional[Blockchain] = None):
    ...
    try:
        from voting_system.distributed_blockchain import register_v2_routes
...
def main() -> None:
    """Start the FastAPI app for local development and presentations."""
    if app is None:
    ...
    target = "voting_system.blockchain_v1:app" if reload else app
    uvicorn.run(target, host="127.0.0.1", port=port, reload=reload)
